        ...
        for comment in comments:
            ...
            if sub_comments:
                 if len(sub_comments) > 5:
                     sub_comments = sub_comments[:5]
                     comment["sub_comment_has_more"] = False # 强制设为没有更多
            ...
            while sub_comment_has_more and sub_count < 5: # 限制总数不超过5
                ...
                if "comments" not in comments_res:
                    ...
                    break
                comments_batch = comments_res["comments"]
                if sub_count + len(comments_batch) > 5:
                    comments_batch = comments_batch[: 5 - sub_count]
                    sub_comment_has_more = False # 停止翻页
                
                sub_count += len(comments_batch)
                if callback:
                    await callback(note_id, comments)
                await asyncio.sleep(crawl_interval)
                result.extend(comments)
